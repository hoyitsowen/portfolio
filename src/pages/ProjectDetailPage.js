  ...
  if (!project) {
    return (
      <div className="min-h-screen p-12">
        ...
        <Link to="/projects" className="text-lime-600">Back to projects</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-black transition-colors duration-500 dark:bg-[#141414] dark:text-white">
      <main className="mx-auto max-w-4xl px-6 py-12 lg:py-20">
        <Link to="/projects" className="text-sm text-lime-600">← Back to projects</Link>

        <header className="my-6">
          <h1 className="text-4xl font-semibold">{project.title}</h1>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{project.category} • {project.year}</p>
        </header>
        ...
        <section className="mb-8">
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {project.tech.map((t) => (
              <li key={t} className="rounded-md border px-3 py-1 text-sm text-gray-700 dark:text-gray-200">{t}</li>
            ))}
          </ul>
        </section>
